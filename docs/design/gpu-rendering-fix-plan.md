...
| 问题              | 现象                      | 根因                                | 状态      |
| ----------------- | ------------------------- | ----------------------------------- | --------- |
| ~~Flow 参数无效~~ | 低 Flow 时 GPU 仍显示纯色 | 所有 dab 同时渲染，无法读取累积结果 | ✅ 已修复 |

**问题分析：**

```
CPU 工作方式 (正确):
  dab1 → 更新 buffer → dab2 读取更新后的 dst_a → 累积 → dab3...
  每个 dab 能看到之前 dab 的结果，Alpha Darken 累积正常

GPU 当前工作方式 (错误):
  [dab1, dab2, dab3, dab4, dab5] → 同一个 render pass
                                → 全部从同一个源纹理读取
                                → dst_a 全是 0 (之前状态)
                                → 每个 dab 都认为是第一次绘制
                                → Alpha Darken 累积失效
...
1. **Compute Shader 方案** (推荐)
   - 顺序处理每个 dab (atomic 或循环)
   - 可实现正确的 Read-Modify-Write
   - 需重写核心渲染逻辑

2. **多 Pass 方案**
   - 每个 dab 单独一个 render pass
   - 每次读取上一帧结果
   - 正确但性能较差

3. **预计算累积表**
   - 在 CPU 预计算重叠区域的最终 alpha
   - 一次性渲染最终状态
   - 适合高重叠场景，复杂度较高
...
| 问题                        | 现象                         | 根因                                     | 状态      |
| --------------------------- | ---------------------------- | ---------------------------------------- | --------- |
| 抬笔闪烁                    | 画完一笔时画面闪烁           | Preview/Composite 使用不同 readback 时机 | ✅ 已修复 |
| ~~**多图层 Preview 层序**~~ | 在下层绘制时笔触显示在最上层 | Preview canvas 独立于图层，合成时才归位  | ✅ 已修复 |

> [!NOTE]
> **修复方案 (2026-01-15)**: 添加 `LayerRenderer.compositeWithPreview()` 方法，
