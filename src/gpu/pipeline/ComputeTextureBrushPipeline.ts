...
export class ComputeTextureBrushPipeline {
  ...
  dispatchStrokeLevelPattern(
    ...
    if (tiles.length > MAX_TILES_PER_BATCH) {
      if (shouldLogUploadDebug()) {
        console.warn('[ComputeTextureBrushPipeline] Too many tiles for stroke-level pattern pass', {
          tiles: tiles.length,
          max: MAX_TILES_PER_BATCH,
          bbox,
        });
    ...
    safeWriteBuffer({
      ...
      srcOffset: 0,
      size: tiles.length * this.uniformStride,
      label: 'ComputeTextureBrush stroke-level pattern uniforms',
    });
    ...
    const pass = encoder.beginComputePass({
      label: 'Compute Texture Brush Stroke-Level Pattern Pass',
    });
    pass.setPipeline(this.pipeline);
