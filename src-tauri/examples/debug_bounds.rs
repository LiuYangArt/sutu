/// Debug VMA search with explicit bounds checking
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for idx in [8, 11, 12] {
        ...
        // Force explicit bounds check
        if pattern.data.len() < 100 {
        ...
        for test_offset in 0..50 {
            // Explicit bounds check
            if test_offset + 31 > pattern.data.len() {
                break;
            }

            let d = &pattern.data[test_offset..test_offset + 31];
        ...
        if !found {
            println!("MISS (checked 50 offsets)");
        }
    }
